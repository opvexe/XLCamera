...
static XLFileManager * manager = nil;
...
+ (id)defaultManager
{
    ...
    dispatch_once(&onceToken, ^{
        manager = [[XLFileManager alloc] init];
    });
...
- (void)conversionFormatMp4WithUrl:(NSURL*)videoUrl videoUrl:(NSString *)url videoblock:(void (^)(BOOL success, NSString* urlString))callblock
{
    ...
    if ([compatiblePresets containsObject:AVAssetExportPresetMediumQuality]) {
        ...
        NSLog(@"视频文件路径:==%@",[NSHomeDirectory() stringByAppendingFormat:@"/Documents/%@.mp4", [formater stringFromDate:[NSDate date]]]);
